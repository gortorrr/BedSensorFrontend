...
export default function EmergencyAlert({ onClose }: EmergencyAlertProps) {
  ...
  const updateStatus = (id: number, accepted?: boolean, successed?: boolean) => {
    setNotifications((prev) =>
      prev.map((noti) =>
        noti.notification_id === id
          ? {
              ...noti,
              notification_accepted: accepted !== undefined ? accepted : noti.notification_accepted,
              notification_successed: successed !== undefined ? successed : noti.notification_successed,
  ...
  const getTimeElapsed = (notificationDate: string | Date): string => {
    const date = typeof notificationDate === "string" ? new Date(notificationDate) : notificationDate;
    const now = new Date();
